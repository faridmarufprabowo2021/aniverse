...
import {
  ...
  User2, Tag, Info, Eye, ArrowUpDown, Palette, Hash, Award
} from "lucide-react";
...
export default function MangaDetailPage() {
  ...
  if (loading) return (
    <>
      ...
      <main className="pt-safe pb-safe min-h-screen flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-[var(--color-text-muted)]">Memuat detail manga...</p>
      </main>
      <BottomNavbar />
    </>
  );
  ...
  // Unified data extraction from either provider
      ...
      <main className="pt-safe pb-safe min-h-screen">
        <div className="max-w-lg mx-auto px-4 pt-4 pb-28 space-y-5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {comic.rating && (
                  <div className="flex items-center gap-0.5">
                    ...
                    {comic.votes && <span className="text-[9px] text-[var(--color-text-muted)]">({comic.votes})</span>}
                  </div>
                )}
              </div>
              ...
              {comic.genres && comic.genres.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {comic.genres.map((g, i) => (
                    <span key={g.name + i} className="text-[9px] px-1.5 py-0.5 rounded-lg bg-[var(--color-surface-2)] border border-[var(--color-border)] text-[var(--color-text-secondary)]">
                      {g.name}
                    </span>
                  ))}
                </div>
          ...
          {chapters.length > 0 && (
            <div className="flex gap-2">
              ...
              <Link href={`/manga/chapter/${encodeURIComponent(latestChapter?.slug || "")}`} className="flex-1">
                <button className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] text-sm font-bold hover:border-blue-500/40 transition-colors">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="text-center p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl">
            ...
            <div className="text-center p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl">
              <p className="text-lg font-black" style={{ color: typeColor }}>{comicType || "—"}</p>
          ...
          {synopsis && (
            <div className="p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl space-y-2">
              ...
              <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed whitespace-pre-line">{synopsis}</p>
            </div>
          )}
                ...
                {displayedChapters.map((ch, i) => (
                  <Link key={ch.slug + i} href={`/manga/chapter/${encodeURIComponent(ch.slug)}`}>
                    <motion.div
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-blue-500/30 hover:bg-blue-500/5 transition-all group"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {ch.releaseTime && (
                          <p className="text-[10px] text-[var(--color-text-muted)] flex items-center gap-0.5">
